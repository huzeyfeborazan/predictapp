...
**This app is using Flask 3.0.3 and Python 3.12.3**. The screenshot doesn't get
...
- **Flask defaults that are changed**:
    ...
    - `ProxyFix` middleware is enabled (check `predictapp/app.py`)
...
```sh
git clone https://github.com/nickjj/docker-flask-example predictapp
cd predictapp
...
The app is named `predictapp` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "predictapp" and update a few Docker related resources.
