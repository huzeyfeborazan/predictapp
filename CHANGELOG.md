...
- In `predictapp/up/views.py`, update raw SQL `SELECT 1` health check for SQLAlchemy 2.0
...
- `predictapp/initializers.py` now exists to define frequently used imports, variables, etc.
...
- `predictapp/blueprints/` namespace has been removed
...
- Import `Redis` more efficiently in `predictapp/extensions.py`
...
- Unnecessary `CELERY_TASK_LIST` comment in `predictapp/app.py`, this was a left over copy / paste mishap
